...
// Full 3-calendar cleanup workflow (RevOps + Blackthorn + Iceberg)
const cleanupWorkflow = {
  ...
  nodes: [
    ...
    {
      parameters: {
        jsCode: 'const data = $input.first().json;\nconst events = data.items || [];\nreturn [{ json: { calendarLabel: "blackthorn", events } }];',
        mode: 'runOnceForAllItems'
      },
      id: 'agg-blackthorn', name: 'Agg Blackthorn',
    ...
    },
    {
      ...
      id: 'fetch-iceberg', name: 'Fetch Iceberg Events',
      type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [1280, 300],
    ...
    },
    {
      parameters: {
        jsCode: [
          ...
          '  const src = getSyncSource(e);',
          '  if (src && ALL_SYNC_TAGS.includes(src)) deleteActions.push({ calendarId: "jort%40icebergops.com", credential: "iceberg", eventId: e.id, summary: e.summary || "(no title)", syncSource: src });',
          '}',
          '',
          'console.log("RevOps synced: " + revopsEvents.filter(e => ALL_SYNC_TAGS.includes(getSyncSource(e))).length);',
          'console.log("Blackthorn synced: " + blackthornEvents.filter(e => ALL_SYNC_TAGS.includes(getSyncSource(e))).length);',
          'console.log("Iceberg synced: " + icebergEvents.filter(e => ALL_SYNC_TAGS.includes(getSyncSource(e))).length);',
    ...
    },
    {
      ...
      type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [2160, 80],
      credentials: { googleCalendarOAuth2Api: { id: 'dZ2lCgJhToJOSI7t', name: 'RevOps Calendar - joe@revopsinflection.com' } }
    },
    {
      parameters: { conditions: { string: [{ value1: '={{ $json.credential }}', operation: 'equal', value2: 'blackthorn' }] } },
      id: 'route-blackthorn', name: 'Is Blackthorn?',
      type: 'n8n-nodes-base.if', typeVersion: 1, position: [2160, 320]
    },
    {
      parameters: {
        method: 'DELETE', url: '={{ $json.url }}',
      ...
      type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [2380, 200],
      credentials: { googleCalendarOAuth2Api: { id: 'xhKqi8iJDl6w5bh6', name: 'Blackthorn Calendar account' } }
    },
    {
      parameters: {
      ...
      type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [2380, 420],
      credentials: { googleCalendarOAuth2Api: { id: 'mNQ6oDJf55C2j6vl', name: 'Iceberg Calendar - jort@icebergops.com' } }
    },
    {
      parameters: {
  ...
  connections: {
    'Webhook Trigger': { main: [[{ node: 'Fetch RevOps Events', type: 'main', index: 0 }]] },
    'Fetch RevOps Events': { main: [[{ node: 'Agg RevOps', type: 'main', index: 0 }]] },
    'Agg RevOps': { main: [[{ node: 'Fetch Blackthorn Events', type: 'main', index: 0 }]] },
    'Fetch Blackthorn Events': { main: [[{ node: 'Agg Blackthorn', type: 'main', index: 0 }]] },
    'Agg Blackthorn': { main: [[{ node: 'Fetch Iceberg Events', type: 'main', index: 0 }]] },
    'Fetch Iceberg Events': { main: [[{ node: 'Build Delete List', type: 'main', index: 0 }]] },
    ...
    'Is RevOps?': { main: [
      ...
      [{ node: 'Is Blackthorn?', type: 'main', index: 0 }]
    ]},
    'Is Blackthorn?': { main: [
...
async function main() {
  ...
  console.log('Deploying full 3-calendar cleanup workflow...');
  ...
  console.log('Activating cleanup workflow...');
  ...
  if (activateResult.status === 200) {
    ...
    console.log('  Activate FAILED HTTP ' + activateResult.status + ': ' + JSON.stringify(activateResult.body).substring(0, 200));
  }
}
